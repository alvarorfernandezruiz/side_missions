...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/login":
            ...
            state["players"] = {}
            save_state(state)
            # Cambio: tras iniciar, ir al panel de admin (estado de partida)
            self.redirect("/admin")
    ...
    def page_admin(self, state):
        # Cambios solicitados:
        # - Si NO hay partida: SOLO botón Iniciar partida.
        # - Si SÍ hay partida: panel de jugadores y SOLO botón Terminar partida.
        if not state["active"]:
            self.html("""
                <h2>Admin</h2>
                <div class="list"><p>No hay partida iniciada.</p></div>
                /start<button>Iniciar partida</button></form>
                /Inicio</a>
            """)
            return
        ...
        self.html(f"""
            <h2>Admin</h2>
            <div class="list">{players or "<p>No hay jugadores aún.</p>"}</div>
            {modals}
    ...
    def html(self, body):
        ...
        self.wfile.write(f"""
            <html><head><style>
            body {{
                background:#0e0e11; color:#fff; font-family:system-ui;
            ...
            .card {{
                background:#f2f2f2; color:#111; padding:20px; margin:12px auto;
            ...
            .player {{
                background:#1e1e2a; padding:16px; border-radius:16px; margin:10px auto;
            ...
            .modal-content {{
                background:#111; padding:24px; border-radius:24px;
